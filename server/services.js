...
async function get_hotels(city) {
  let google_places_api_endpoint =
  ...
  for (let i = 0; i < raw_data.length; i++) {
    ...
    if (raw_data[i].hasOwnProperty("photos") === true) {
      new_object.photo_reference =
        "https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=" +
        raw_data[i].photos[0].photo_reference +
        "&key=" +
        google_places_api_key;
    } else {
      new_object.photo_reference =
        "https://static.nationalgeographic.co.uk/files/styles/image_3200/public/reine-autumn-4.jpg?w=400&h=400&q=75";
    }
  ...
  for (let i = 0; i < only_names.length; i++) {
    ...
    for (let j = 0; j < hotel_prices.length; j++) {
      if (current_hotel.includes(hotel_prices[j].name)) {
        ...
        filtered_hotels[filtered_hotels.length - 1].price =
          hotel_prices[j].average_cost_per_night;
      }
    }
  } // console.log(filtered_hotels); // console.log(result_array);
...
async function get_restaurants(city) {
  ...
  for (let i = 0; i < raw_data.length; i++) {
    ...
    new_object.num_ratings = raw_data[i].user_ratings_total; // if new_object.price is NaN, continue...
...
async function get_attractions(city, attraction_type) {
  ...
  for (let i = 0; i < raw_data.length; i++) {
    ...
    new_object.name = raw_data[i].name;
    new_object.rating = raw_data[i].rating;
    new_object.address = raw_data[i].formatted_address; // new_object.photo_reference = raw_data[i].photos[0].photo_reference;
    new_object.place_id = raw_data[i].place_id;
    new_object.num_ratings = raw_data[i].user_ratings_total;
    ...
    let attraction_types = raw_data[i].types; // console.log(new_object.name); // console.log(attraction_types); // if attraction_types contains one of the attraction types in attractions_prices, add it. // otherwise, set default price to 0 // default value
